...
export const swap = async (walletProvider, amount, isGreat) => {
  ...
  let approveContract;
  if (isGreat) {
    approveContract = new ethers.Contract(
      GreatLoongAddress,
      GreatLABI.abi,
      signer
    );
...
export const getInviteRewards = async (walletProvider, isGreateL) => {
  ...
  const signer = await ethersProvider.getSigner();
  const contract = new ethers.Contract(
    isGreateL ? greatLMintAddr : babyLMintAddr,
    MintABI.abi,
    signer
  );
...
export const mint = async (
  ...
  const realPrice = new Decimal(price || '0')
...
// getBalanceWallet
export const getBalanceWallet = async (walletProvider) => {
  const ethersProvider = new ethers.BrowserProvider(walletProvider);
  const signer = await ethersProvider.getSigner();
